...
def test_scene_snapshot_defaults(monkeypatch):
    ...
    result = registry.call_tool("blender-diag-scene-snapshot", {})
    assert result["ok"] is True
    assert captured["tool"] == "blender-diag-scene-snapshot"
...
def test_object_snapshot_arguments_alias(monkeypatch):
    ...
    try:
        resp = requests.post(
            ...
            json={"name": "blender-diag-object-snapshot", "arguments": {"name": "Cube"}},
            timeout=2,
        )
        data = resp.json()
        assert data["ok"] is True
        assert captured["tool"] == "blender-diag-object-snapshot"
